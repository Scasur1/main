 ...
 * - HİÇBİR ŞEY DEĞİŞMEZ — mapper.data_source, mapper.select ("data_source_item"),
 *   metadata hepsi orijinal kalır. "page" yapılırsa filtreler kayboluyor.
 */
 ...
 * - HİÇBİR ŞEY DEĞİŞMEZ — modül adı, mapper.data_source, metadata hepsi
 *   orijinal kalır. Make.com bu modülü data_source field'larıyla tanır.
 *   database'e çevrilirse "Module Not Found" hatası verir.
 */
function convertCreateDataSourceItem(mod) {
  // Modülü olduğu gibi döndür — hiçbir dönüşüm yapma
  return JSON.parse(JSON.stringify(mod));
}
...
function validate(originalJson, convertedJson, fileName) {
  ...
  // 2. searchObjects1 ve createDataSourceItem modüllerinde data_source korunmuş mu?
  const notionModules = findAllNotionModules(convertedJson);
  const noOpModules = ['notion:createDataSourceItem', 'notion:searchObjects1'];
  const noOpWithDS = notionModules.filter(m =>
    noOpModules.includes(m.module) && 'data_source' in (m.mapper || {})
  );
  const noOpWrong = notionModules.filter(m =>
    noOpModules.includes(m.module) && 'database' in (m.mapper || {}) && !('data_source' in (m.mapper || {}))
  );
  if (noOpWrong.length > 0) {
    issues.push(`❌ ${noOpWrong.length} modülde mapper.data_source yanlışlıkla database'e çevrilmiş`);
  } else if (noOpWithDS.length > 0) {
    info.push(`✅ searchObjects1 ve createDataSourceItem modüllerinde mapper.data_source korundu (${noOpWithDS.length} modül)`);
  }

  // 3. update/get modüllerinde data_source→database dönüşümü yapılmış mı?
  const origNotionModules = findAllNotionModules(originalJson);
  const convertedModuleTypes = ['notion:updateADatabaseItem', 'notion:getADatabaseItem'];
  const origConverted = origNotionModules.filter(m =>
    convertedModuleTypes.includes(m.module) && 'data_source' in (m.mapper || {})
  );
  const convMissing = notionModules.filter((m, i) => {
    if (!convertedModuleTypes.includes(m.module)) return false;
    const orig = origNotionModules[i];
    return orig && 'data_source' in (orig.mapper || {}) && !('database' in (m.mapper || {}));
  });
  if (convMissing.length > 0) {
    issues.push(`❌ ${convMissing.length} update/get modülde data_source→database dönüşümü eksik`);
  } else {
    info.push(`✅ update/get modüllerinde data_source→database dönüşümü tamam (${origConverted.length} modül)`);
  }
