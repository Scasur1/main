...
/**
 * TİP 2: notion:createDataSourceItem → notion:createADatabaseItem
 * - module adı değişir
 * - mapper.data_source → mapper.database
...
function convertCreateDataSourceItem(mod) {
  ...
  m.module = 'notion:createADatabaseItem';
...
function validate(originalJson, convertedJson, fileName) {
  ...
  // 4. notion:createDataSourceItem kalmamış mı?
  const oldCreate = notionModules.filter(m => m.module === 'notion:createDataSourceItem');
  if (oldCreate.length > 0) {
    issues.push(`❌ Hala ${oldCreate.length} adet notion:createDataSourceItem var`);
  } else {
    info.push('✅ notion:createDataSourceItem kalmadı (hepsi createADatabaseItem oldu)');
  }
