...
function validate(originalJson, convertedJson, fileName) {
  ...
  // 2. "data_source" mapper key kalmamış mı? (createDataSourceItem hariç — onlar data_source kalır)
  const notionModules = findAllNotionModules(convertedJson);
  const withDataSource = notionModules.filter(m =>
    'data_source' in (m.mapper || {}) && m.module !== 'notion:createDataSourceItem'
  );
  if (withDataSource.length > 0) {
    ...
    info.push('✅ data_source→database dönüşümü doğru (createDataSourceItem hariç)');
  }

  // 3. createDataSourceItem modüllerinde data_source korunmuş mu?
  const createDSWithDataSource = notionModules.filter(m =>
    m.module === 'notion:createDataSourceItem' && 'data_source' in (m.mapper || {})
  );
  const createDSWithDatabase = notionModules.filter(m =>
    m.module === 'notion:createDataSourceItem' && 'database' in (m.mapper || {}) && !('data_source' in (m.mapper || {}))
  );
  if (createDSWithDatabase.length > 0) {
    issues.push(`❌ ${createDSWithDatabase.length} createDataSourceItem modülünde mapper.data_source yanlışlıkla database'e çevrilmiş`);
  } else if (createDSWithDataSource.length > 0) {
    info.push(`✅ createDataSourceItem modüllerinde mapper.data_source korundu (${createDSWithDataSource.length} modül)`);
  }

  // 3b. Diğer modüllerde data_source→database dönüşümü yapılmış mı?
  const origNotionModules = findAllNotionModules(originalJson);
  const origWithDataSource = origNotionModules.filter(m =>
    'data_source' in (m.mapper || {}) && m.module !== 'notion:createDataSourceItem'
  );
  const convWithoutDatabase = notionModules.filter((m, i) => {
    if (m.module === 'notion:createDataSourceItem') return false;
    const orig = origNotionModules[i];
    return orig && 'data_source' in (orig.mapper || {}) && !('database' in (m.mapper || {}));
  });
  if (convWithoutDatabase.length > 0) {
    issues.push(`❌ ${convWithoutDatabase.length} modülde data_source→database dönüşümü eksik`);
  } else {
    info.push(`✅ Diğer modüllerde data_source→database dönüşümü tamam (${origWithDataSource.length} modül)`);
  }
